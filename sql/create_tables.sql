...
CREATE TABLE orange_produce.blocks(
    ...
    created_by      bigint REFERENCES orange_produce.users ON DELETE CASCADE,
    crops           bigint REFERENCES orange_produce.crops ON DELETE CASCADE,
    primary_key(id)
);
...
CREATE TABLE orange_produce.users_audit(
    id              bigint REFERENCES orange_produce.users ON DELETE CASCADE,
    created_date    timestamptz NOT NULL,
    modified_date   timestamptz NOT NULL, 
);